...
- **`date`**: `int` - Tanggal Jawa.
- **`day`**: `int` - Indeks hari (0=Minggu, 6=Setu).
- **`month`**: `int` - Indeks bulan (0=Sura).
- **`year`**: `int` - Tahun Jawa.
- **`pasaran_index`**: `int` - Indeks pasaran (0=Legi).
- **`neptu`**: `int` - Total nilai neptu.
- **`wuku_index`**: `int` - Indeks wuku (0=Sinta).
- **`mongso_index`**: `int` - Indeks mongso (0=Kasa).
- **`to_datetime() -> datetime`**: Mengembalikan objek `datetime` native.
